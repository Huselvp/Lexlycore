...
import AddEditSubQoice from "./AddEditSubQoice";
import ConfirmeDeleteSubCoice from "./ConfirmeDeleteSubCoice";

const SubCoiceRow = ({ choice }: { choice: Choice }) => {
  return (
    <Table.Row id={`menus-row--${choice.id}`}>
      ...
      <Modal.Window name={`edit-choice-${choice.id}`}>
        <AddEditSubQoice onAdd={false} choice={choice} />
      </Modal.Window>
      <Modal.Window name={`delete-choice-${choice.id}`}>
        <ConfirmeDeleteSubCoice choiceId={choice.id} />
      </Modal.Window>
    </Table.Row>
  );
};

export default SubCoiceRow;
